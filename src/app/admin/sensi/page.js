...
export default function AdminSensi() {
    const { searchQuery, selectedBatch, availableBatches, refreshBatches } = useAdminContext();
    ...
    const fetchStaff = async () => {
        ...
        try {
            ...
            const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL || ''}/api/user/sensi`, {
                headers: { 'Authorization': `Bearer ${token}` }
            });
            if (!res.ok) throw new Error('Failed to fetch sensi');
    ...
    const handleAddLevelTag = (levelToAdd) => {
        const trimmed = (levelToAdd || newLevelInput).trim();
        if (!trimmed) return;
        if (!formData.levels.includes(trimmed)) {
            setFormData(prev => ({ ...prev, levels: [...prev.levels, trimmed] }));
        }
        setNewLevelInput('');
    };

    const handleRemoveLevelTag = (levelToRemove) => {
        setFormData(prev => ({
            ...prev,
            levels: prev.levels.filter(l => l !== levelToRemove)
        }));
    };
    ...
    const handleAddSubmit = async (e) => {
        ...
        try {
            ...
            if (!res.ok) {
                let errText = 'Failed to add sensi member';
                try {
                    const data = await res.json();
            ...
            setFormData({ name: '', email: '', password: '', levels: ['Level 5'], batches: selectedBatch && selectedBatch !== 'All Batches' ? [selectedBatch] : [] });
            setNewBatchInput('');
            setNewLevelInput('');
        } catch (err) {
    ...
    const handleEditSubmit = async (e) => {
        ...
        try {
            const token = localStorage.getItem('token');
            const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL || ''}/api/user/sensi/${currentStaff.id}`, {
                method: 'PUT',
                headers: {
                    'Content-Type': 'application/json',
                    'Authorization': `Bearer ${token}`
            ...
            if (!res.ok) {
                let errText = 'Failed to update sensi member';
                try {
                    const data = await res.json();
                    errText = data.detail || errText;
                } catch {
    ...
    const handleDelete = async () => {
        try {
            ...
            if (!res.ok) throw new Error('Failed to delete sensi member');
            await fetchStaff();
    ...
    const openEditModal = (staffMember) => {
        ...
        const existingLevels = staffMember.levels && staffMember.levels.length > 0
            ? staffMember.levels
            : (staffMember.level ? [staffMember.level] : ['Level 5']);
        setFormData({ 
        ...
        return (
            <section className="p-gutter max-w-[1440px] mx-auto min-h-[50vh] flex items-center justify-center">
                <div className="flex flex-col items-center gap-4 text-primary">
                    ...
                    <p className="font-label-lg">Loading Sensi...</p>
                </div>
            </section>
        );
    }
    ...
    return (
        <div className="max-w-[1440px] mx-auto p-gutter space-y-lg relative pb-32 animate-fade-in">
            <section className="flex flex-col md:flex-row md:items-center justify-between gap-md mb-lg">
                <div>
                    ...
                    <p className="font-body-md text-on-surface-variant max-w-2xl">
                        View, add, edit, and assign multiple batches to sensi accounts.
                    </p>
                </div>
                <button 
                    onClick={() => { setFormData({ name: '', email: '', password: '', levels: ['Level 5'], batches: selectedBatch && selectedBatch !== 'All Batches' ? [selectedBatch] : [] }); setFormError(''); setNewBatchInput(''); setNewLevelInput(''); setIsAddModalOpen(true); }}
                    ...
                    <table className="w-full text-left border-collapse min-w-[580px]">
                        ...
                        <tbody>
                            {filteredStaff.map(staffMember => {
                                const bList = staffMember.batches && staffMember.batches.length > 0 ? staffMember.batches : (staffMember.batch ? [staffMember.batch] : []);
                                const lList = staffMember.levels && staffMember.levels.length > 0 ? staffMember.levels : (staffMember.level ? [staffMember.level] : ['Level 5']);
                                return (
                                            ...
                                            <div className="flex flex-wrap gap-1.5 items-center">
                                                {lList.map((lvl, idx) => (
                                                    <span key={idx} className={`px-2 py-1 rounded-full text-xs font-medium ${
                                                        lvl === 'Level 5' ? 'bg-blue-100 text-blue-800' :
                                                        lvl === 'Level 4' ? 'bg-purple-100 text-purple-800' :
                                                        lvl === 'Level 3' ? 'bg-orange-100 text-orange-800' :
                                                        lvl === 'Level 2' ? 'bg-green-100 text-green-800' :
                                                        lvl === 'Level 1' ? 'bg-red-100 text-red-800' :
            ...
            {isAddModalOpen && (
                <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/50 backdrop-blur-sm p-4">
                    <div className="bg-surface rounded-2xl p-xl w-[90%] max-w-[540px] shadow-2xl max-h-[90vh] overflow-y-auto custom-scrollbar">
                        <h2 className="font-headline-md text-on-surface mb-md">Add New Sensi</h2>
                        ...
                        <form onSubmit={handleAddSubmit} className="space-y-4">
                            ...
                            <div>
                                <label className="block text-label-md text-on-surface-variant mb-1">Assigned Levels (Multiple)</label>
                                <div className="flex flex-col sm:flex-row gap-2 mb-2">
                                    <input 
                                        type="text" 
                                        placeholder="e.g. Level 1"
                                        className="flex-1 bg-surface-container-low border border-outline-variant rounded-lg p-2.5 sm:p-3 text-on-surface focus:outline-none focus:border-primary transition-colors text-sm"
                                        value={newLevelInput}
                                    ...
                                    <button 
                                        type="button" 
                                        onClick={() => handleAddLevelTag()}
                                        className="px-4 py-2.5 bg-primary/10 text-primary hover:bg-primary/20 border border-primary/30 rounded-lg text-xs sm:text-sm font-semibold transition-colors flex items-center justify-center gap-1 shrink-0 cursor-pointer active:scale-95"
                                    >
                                ...
                                {/* Active Assigned Levels Chips */}
                                <div className="flex flex-wrap gap-1.5 sm:gap-2 min-h-[38px] p-2 bg-surface-container-low border border-outline-variant/50 rounded-lg items-center">
                                    {formData.levels && formData.levels.length > 0 ? (
                                        formData.levels.map((l, idx) => (
                                            <span key={idx} className="bg-primary text-on-primary px-2.5 sm:px-3 py-1 rounded-full text-xs font-medium flex items-center gap-1.5 shadow-sm">
                                                <span>{l}</span>
                                                <button 
                                                    type="button" 
                                                    onClick={() => handleRemoveLevelTag(l)} 
                                        ...
                                        <span className="text-xs text-on-surface-variant/70 italic">No levels assigned yet. Enter level above or click quick suggestions below.</span>
                                    )}
                                </div>
                                ...
                                <div className="flex flex-wrap gap-1.5 mt-2 items-center">
                                    ...
                                    {['Level 1', 'Level 2', 'Level 3', 'Level 4', 'Level 5'].map((l) => (
                                        <button 
                                            key={l}
                                            type="button"
                                            onClick={() => handleAddLevelTag(l)}
                                            disabled={formData.levels.includes(l)}
                                            className="text-[11px] px-2.5 py-1 rounded-full border border-outline-variant hover:bg-surface-container-high text-on-surface disabled:opacity-40 disabled:cursor-not-allowed transition-colors active:scale-95"
                                        >
                                            + {l}
                                        </button>
                                    ))}
                            ...
                            <div className="flex justify-end gap-3 mt-6">
                                <button 
                                    ...
                                    className="px-5 sm:px-6 py-2 rounded-full text-on-surface-variant hover:bg-surface-container transition-colors text-xs sm:text-sm"
                                >
                                    Cancel
                                </button>
                                <button 
                                    type="submit"
                                    className="px-5 sm:px-6 py-2 rounded-full bg-primary text-on-primary hover:bg-primary/90 transition-colors shadow-md text-xs sm:text-sm font-semibold"
                                >
                                    Add Sensi
                                </button>
                            </div>
            ...
            {isEditModalOpen && (
                <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/50 backdrop-blur-sm p-4">
                    <div className="bg-surface rounded-2xl p-5 sm:p-xl w-[95%] sm:w-[90%] max-w-[540px] shadow-2xl max-h-[90vh] overflow-y-auto custom-scrollbar">
                        <h2 className="font-headline-md text-on-surface mb-md">Edit Sensi</h2>
                        {formError && <p className="text-error mb-4 text-sm bg-error-container p-2 rounded">{formError}</p>}
                ...
                <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/50 backdrop-blur-sm p-4">
                    <div className="bg-surface rounded-2xl p-xl w-[90%] max-w-[500px] shadow-2xl">
                        <div className="flex flex-col items-center text-center gap-4">
                            ...
                            <h2 className="font-headline-md text-on-surface">Delete Sensi Member?</h2>
