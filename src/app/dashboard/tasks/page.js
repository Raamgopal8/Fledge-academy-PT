...
export default function StudentTasks() {
    ...
    const handleSubmit = async (taskId) => {
        if (!submissionContent.trim() || !studentName.trim()) {
            alert('Please provide both your name and submission content.');
            return;
        }
                                ...
                                {isLoading ? (
                                    ...
                                    displayedTasks.map(task => (
                                        <div key={task.id} className="bg-surface-container-lowest rounded-xl p-md border border-outline-variant shadow-sm hover:shadow-md transition-shadow">
                                            ...
                                            {/* Action Area */}
                                            {activeTab === 'pending' && task.submission && task.submission.status === 'Needs Work' && (
                                                <div className="mt-md pt-md border-t border-outline-variant bg-error-container/20 -mx-md p-md rounded-xl mb-md border-error/20">
                                                    <h4 className="font-label-md text-error flex items-center gap-xs mb-xs">
                                            ...
                                            {activeTab === 'pending' && (
                                                <div className="mt-md pt-md border-t border-outline-variant">
                                                    <input 
                                                        type="text"
                                                        className="w-full p-sm rounded-lg border border-outline focus:border-primary focus:ring-1 focus:ring-primary outline-none bg-surface-container-lowest text-on-surface mb-sm"
                                                    ...
                                                    <div className="flex justify-end">
                                                        <button 
                                                            ...
                                                            disabled={submittingId === task.id || !submissionContent.trim() || !studentName.trim()}
                                                        ...
                                                        >
                                                            ...
                                                            {task.submission ? "Resubmit Task" : "Submit Task"}
                                                        </button>
                                                    </div>
                                                </div>
                                            )}
